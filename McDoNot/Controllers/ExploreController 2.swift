...
let testWifi = Place(accessibility: false, address: "Dioc", foodSelling: false, freeParking: false, ID: "idcazzo", imageURL: "cococo", isSilent: false, latitude: 40.836760, longitude: 14.307642, name: "panePane", plug: false, rating: 3, ratingCount: 1, smokingArea: false, type: "Bar", vendingMachine: false, wiFi: true)

let testWifiAndSmoke = Place(accessibility: false, address: "Dioc", foodSelling: false, freeParking: false, ID: "culi", imageURL: "cococo", isSilent: false, latitude: 40.836091, longitude: 14.305006, name: "simplySimply", plug: false, rating: 3, ratingCount: 1, smokingArea: true, type: "Bar", vendingMachine: false, wiFi: true)
...
class ExploreController: UIViewController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    var tagBools = [false, false, false, false, false, false, false] {
        didSet {
            ...
            //            mapView.showAnnotations(getAnnotationsFromPlaces(Places: testingShownLocation), animated: true)
    ...
    private func setupUserTrackingButtonAndScaleView() {
        ...
        NSLayoutConstraint.activate([
            stackView.bottomAnchor.constraint(equalTo: self.mapView.bottomAnchor, constant: -10),
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if collectionView == tagCollection {
            ...
            cell.placeTag.layer.borderColor = UIColor(red: 60.0 / 255, green: 58.0 / 255, blue: 114.0 / 255, alpha: 1.0).cgColor
            cell.placeTag.layer.borderWidth = 1.0
            cell.placeTag.layer.cornerRadius = 6
            //            cell.placeTag.frame.size = CGSize(width: cell.placeTag.intrinsicContentSize.width+50, height: cell.placeTag.intrinsicContentSize.height)
            cell.placeTag.addConstraint(.init(item: cell.placeTag!, attribute: .width, relatedBy: .equal, toItem: nil, attribute: .notAnAttribute, multiplier: 1, constant: cell.placeTag.intrinsicContentSize.width+12))
            cell.placeImage.loadImage(fromURL: tempElement.imageURL)
    ...
    func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if let myCell = cell as? PlaceCollectionCell {
            let lattitude = myCell.place!.latitude
            let longitude = myCell.place!.longitude
